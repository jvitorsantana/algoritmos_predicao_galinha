...
# SMOTE augmentation for regression:
# Include y as the last column so synthetic samples get interpolated y values
print("\nApplying SMOTE augmentation to training set...")
y_bins = pd.qcut(y_train, q=5, labels=False, duplicates='drop')
print(f"Weight bins distribution:\n{y_bins.value_counts().sort_index()}")
Xy_train = np.column_stack([X_train.values, y_train.values])
smote = SMOTE(random_state=42, k_neighbors=3)
n_before = len(X_train)
Xy_aug, _ = smote.fit_resample(Xy_train, y_bins)
X_train = pd.DataFrame(Xy_aug[:, :-1], columns=FEATURES)
y_train = pd.Series(Xy_aug[:, -1], name='PESO')
print(f"SMOTE: {n_before} -> {len(X_train)} training samples")
